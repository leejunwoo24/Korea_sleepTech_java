...
/*
 	...
 	: 자바에서는 "함수"의 개념 X
 	: 모든 함수 체계는 "클래스 내부"에 존재 >> 메서드로 불린다.
 	...
 	cf) double(8byte) - int(4)
 	
 	double addDouble(int a, int b) {
 		int c, d;// - 지역변수 // 이렇게 선언 가능 but 매개변수는 불가능
...
//		example.add("1", "2"); - 인자값과 매개변수의 타입은 같거나 자동 형 변환이 가능한 것이어야 한다.
