...
public class Exception03 {
	...
	// 1) 단일 예외 던지기 throws
	// : 메서드 선언부에서 해당 메서드가 발생시킬 수 있는 예외 명시하는 키워드
	// - 메서드 호출자에게 예외처리를 위임
	static void login(String username, String password) throws InvalidUserException{
		...
		if(!"admin".equals(username)) {
			// 잘못된 사용자인 경우
			throw new InvalidUserException("사용자 이름이 잘 못 되었습니다.");
		}
	}
	...
	static void exceptions(int num) throws CustomException, ArithmeticException, ArrayIndexOutOfBoundsException {
		if(num == 1) {
			...
			throw new ArrayIndexOutOfBoundsException("그 외에는 - 배열 인텍스 초과 예외");
		}
	}
	...
	public static void main(String[] args) {
		...
		// : throws로 메서드의 예외 발생 가능성을 명시하는 경우 반드시! try-catch 블록의 예외처리가 필수
		try {
		...
		// === 여러 예외 던지기 === //
		try {
			System.out.println("어러 예외 던지기 테스트");
			exceptions(1);
			...
			System.out.println("3번:" + e.getMessage());
		}
	}
}
