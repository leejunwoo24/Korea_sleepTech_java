...
public class StudentManagementApp {
	...
	//  >> main 메ㅓ서드 내부에서 다른 메서드 호출 시 반드시 해당 메서드는 static이여야 함
	...
	static void addStudent(int id, String name, int age, String major, double gpa) {
		...
		studentList.add(student); // .add(리스트의 요소 타입과 일치한 데이터만 삽입가능)
		//numberList.add(100);
		System.out.println("학생이 등록되었습니다..");
	}
	
	// 모든 학생 출력 
	static void printAllStudnets() {
	...
	static void updateStudentGpa(int studentId, double newGpa) {
		...
		System.out.println("해당 ID의 학생을 찾을 수 없습니다. 업데이트 불가!");
	}
	
	//main 메서드
	// : 프로그램의 진입점을 제공
	// >> public static void main(String[] args) 형식이여야만 실행 가능
	// >> JVM이 main 메서드가 없을 경우 해당 파일을 실행하지 X
	public static void main(String[] args) {
		Student student1 = new Student(1, "이승아", 30, "슬립테그", 4.0);
//		Student student2 = new Student(2, "이도경", 31, "슬립테그", 3.1);
//		Student student3 = new Student(3, "이지희", 20, "슬립테그", 4.2);
...
//		studentList.add(student4);
		...
		// 학생 추가
		addStudent(1, "이승아", 30, "슬립테그", 4.0);
		addStudent(2, "이도경", 31, "슬립테그", 3.1);
		addStudent(3, "이지희", 20, "슬립테그", 4.2);
		addStudent(4, "이지훈", 17, "슬립테그", 3.5);
