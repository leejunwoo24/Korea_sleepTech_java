...
class A{
	A(){}
	A(int a){
		this(); //사용자 정의 생성자 내에서 다른 생성자 호출은 반드시 최상단에서 이루어져야 한다!!
...
/*
 	...
 	1) 필드: name(이름)
 	2) 생성자: 매개변수 X 생성자, 매개변수 O 생성자(String name) - 오버로디ㅏㅇ
...
class Cat extends Mammal{
	...
	Cat(String name){
		...
		// >> 여러개의 생성자가 있는 경우(오버로딩) super의 형태도 다양할 수 있음
		
//		this.name = name;
	}
	
	void displayCat() {
		// 해당 클래스가 가진 인스턴스 변수에 접: this
		// >> 생략 가능(지역 변수와 충돌나지 않는 경우)
		System.out.println(name); 		//=> 이 둘은 동일
		System.out.println(this.name);
		
		//부모 클래스가 가진 인스턴스 변수에 접근: supper
		// >> 생략 불가!!
