...
class Order {
	private String orderId; // 각 주문의 식별 번호
...
interface PaymentProccesser{
	void processPayment(Order order); // 주문 객체를 받아 결제를 처리하는 메서드
}

class CreditCardPaymentProcessor implements PaymentProccesser{
	@Override
...
class SamsungPayPaymentProcessor implements PaymentProccesser{
	@Override
	public void processPayment(Order order) {
...
class OrderService {
	private final PaymentProccesser paymentProcessor;
	private final GenerateReport generateReport;
	
	public OrderService(PaymentProccesser paymentProcessor, GenerateReport generateReport) {
...
public class C_OOP_Prac {
	public static void main(String[] args) {
		...
		PaymentProccesser creditCardpaymentProcessor = new CreditCardPaymentProcessor();
		PaymentProccesser samsungPayPaymentProccesser = new SamsungPayPaymentProcessor();
		...
		OrderService orderService2 = new OrderService(samsungPayPaymentProccesser, generateReport);
