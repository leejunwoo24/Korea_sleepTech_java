...
class DiscountPolicy{
	public static double applyDiscount(List<? extends Product> products, double discountRate) {
		//리스트 컬렉션 프레임워크 형식에 Product 타입을 가지거나 해당 타입의 하위 클래스만을 허용
		// : 상한 제한 와일드 카드
...
public class C_Generic_Prac {
	public static void main(String[] args) {
		...
		System.out.println("총 가전 할인액: " + totalElectronics + "원"); // 총 가전 할인액: 31.8
		
		List<Furniture> furnitures = new ArrayList<>();
		furnitures.add(new Furniture("침대", 230));
		furnitures.add(new Furniture("쇼파", 170));
		furnitures.add(new Furniture("화장댜", 50));
		
		double totalFurniures = DiscountPolicy.applyDiscount(furnitures, 0.2);
		
		System.out.println("총 가구 할인액: " + totalFurniures + "원"); // 총 가구 할인액 : 90.0dnjs
	}
}
