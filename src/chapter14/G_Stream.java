...
// == 1. 특징 == //
// 1) 스트림은 데이터 소스로 부터 연속적으로 처리하는 '파이프라인'을 구축
...
// 3) 최종 연산
// : 모든 데이터를 처리한 후 결과를 반화나거나 출력하는 연산
...
public class G_Stream {
	public static void main(String[] args) {
		...
		names.stream()
				.map(name -> name + "님")
				// 문자열.startWith(""): 해당 키워드로 시작하는 문자일 경우 true를 반환
				.filter(name -> name.startsWith("이"))
//				.forEach(System.out::println);
				.forEach(name -> System.out.println(name)); // 위에와 동일
