...
// : 다른 클래스 내부에 선언된 클래스 의미

// == 장점 ==
// : 코드의 가독성 향상 - 코드의 구조 파악 용이
// : 클래스간의 관계를 명확하게 표현 - 내부 클래스는 외부 클래스의 멤버(필드, 메서드)에 접근 가능
 ...
 * 			>> 외부 클래스의 정적 멤버에만 접근 O (외부 클래스의 인스턴스 생성 없이 인스턴스화가 가능)
...
public class A_Inner {
	public static void main(String[] args) {
		...
		// 1) 외부 클래스 인스턴스화 필요 X, 클래슬를 사용한 내부 클래스 생성 가능
		...
		// 클래스타입 변수명 = new 클래스명() {메서드 구현 - 추상클래스/인터페이스의 추상}
		AbstractClass abstractClass1 = new AbstractClass() {
			
			@Override
			void display() {
				System.out.println("익명 내부 클래스 - 추상 클래스");
