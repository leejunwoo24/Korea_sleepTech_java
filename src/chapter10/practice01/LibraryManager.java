...
public class LibraryManager implements Manageable {
	...
	public List<Item> search(String keyword) {
		...
		for(Item item: items) {
			if(item.getName().contains(keyword)
					|| (item instanceof Book && ((Book)item).getAuthor().contains(keyword) || ((Book)item).getPublish().contains(keyword) )
			) {
				foundItems.add(item);
			}
		}
	...
	@Override
	public List<Item> searchByPrinceRange(int minPrice, int maxPrice) {
		List<Item> result = new ArrayList<>();
