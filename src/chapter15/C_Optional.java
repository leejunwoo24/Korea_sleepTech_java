...
/*
 * === Optional 클래스 ===
 * : 값이 존재할 수 잇고, 없을 수도 있는 객체를 감싸는 컨테이너 (자바 8 이후)
 * - null 값을 다룰 때 발생하는 NullPointerExceptio(NPE) 문제를 줄이고
...
//				- null 전달 시 빈 Optional 객체를 반환
		...
		try {
			System.out.println(optional3.orElseThrow(() -> new Error("사용자 정의 예외 발생")));
		} catch(Error e){
...
//		cf) Optional 사용 시 주의점
		...
		// - 값이 반드시 존재해야하는 경우 Optional 사용 X
		// 		: null 방지에 대한 필수 값은 직접 값을 반환하고 null 여부를 확인
